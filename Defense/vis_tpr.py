...
import torch
import shutil
from realtabformer_generate import train_model, evaluate_synthetic_size_tpr, evaluate_vanilla_model
...
def main(base_dir: str):
    ...
    tpr_threshold = 0.1 * 1.25   # <-- switched from AUC threshold to TPR threshold
    max_tendency = 20.0
    ...
    # Create train_test_synth_tpr directory and save dataframes
    train_test_dir = os.path.join(base_directory, "train_test_synth_tpr")
    ...
    for num_synthetic_record in synthetic_sizes:
        ...
        try:
            results, synth_df = evaluate_synthetic_size_tpr(
                member_df=member_df,
            ...
            if synth_df is not None:
                synth_filename = os.path.join(train_test_dir, f"synth_tpr_{num_synthetic_record}.csv")
                try:
            ...
            if opt is None:
                # threshold not met; use best (lowest TPR) among tried points
                if results['tendency_results']:
                    opt = min(results['tendency_results'], key=lambda x: x['tpr_at_fpr_0.1'])['tendency']
                else:
    ...
    if vanilla_synth is not None:
        ...
        vanilla_filename = os.path.join(train_test_dir, f"vanilla_tpr_{max_size}.csv")
        try:
...
def save_summary_csv(all_results, base_dir: str):
    ...
    for result in all_results:
        row_data = {
            ...
            'final_tpr': result.get('final_tpr'),
            'mmd_score': None,
    ...
    csv_filename = os.path.join(summary_dir, f"summary_tpr_{os.path.basename(base_dir)}.csv")
...
def save_vanilla_summary_csv(vanilla_results, base_dir: str):
    ...
    for result in vanilla_results:
        row_data = {
            ...
            'tpr_at_fpr_0.1': result.get('tpr_at_fpr_0.1'),
            'mmd_score': None,
            'jensen_shannon_score': None,
            'wasserstein_score': None,
    ...
    # Save CSV file in summary directory
    csv_filename = os.path.join(summary_dir, f"vanilla_summary_tpr_{os.path.basename(base_dir)}.csv")
    df.to_csv(csv_filename, index=False, float_format='%.6f')
    ...
    for dataset_name, all_results in ALL_RESULTS:
        ...
        for result in all_results:
            ...
            print(f"{status} Synthetic size: {result['num_synthetic_record']:5d} | "
                  ...
                  f"Final TPR: {result['final_tpr']:.4f}{fidelity_str}")
