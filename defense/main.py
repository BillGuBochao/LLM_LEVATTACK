...
# Import the unified privacy evaluation module
import tlp.run_tlp as run_tlp
from tlp.compare_visualization import compare_experiment_results_auc, compare_experiment_results_tpr
...
def main(base_dir: str, eval_type: Literal["auc", "tpr"]):
    ...
    try:
        # Step 1: Run the unified evaluation pipeline
        print(f"\n{'-'*60}")
        print(f"STEP 1: Running {eval_type.upper()} evaluation pipeline")
        print(f"{'-'*60}")
        
        # Call the unified vis_privacy module with the appropriate eval_type
        print(f"Executing {eval_type.upper()}-based privacy evaluation...")
        all_results = run_tlp.main(base_dir, eval_type=eval_type)
        ...
        expected_files = [
            f"summary/{eval_type}/summary_{eval_type}_{dataset_name}.csv",
            f"summary/{eval_type}/vanilla_summary_{eval_type}_{dataset_name}.csv", 
            f"comparison_{eval_type}_plot_{dataset_name}.png"
        ]
...
Output files:
    For AUC evaluation:
        - summary/summary_auc_{dataset}.csv: Tendency-based results
        - summary/vanilla_summary_auc_{dataset}.csv: Vanilla baseline results  
        - comparison_auc_plot_{dataset}.png: Visual comparison
        
    For TPR evaluation:
        - summary/summary_tpr_{dataset}.csv: Tendency-based results
        - summary/vanilla_summary_tpr_{dataset}.csv: Vanilla baseline results
