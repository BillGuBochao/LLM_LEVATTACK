...
class CustomLogitsProcessor(LogitsProcessor):
    """
    Custom logits processor to normalize and apply power transformation
    for controlling sampling behavior uniformity.
    """
    
    def __init__(self, tendency: float = 100.0, epsilon: float = 0):
        self.tendency = tendency
        self.epsilon = epsilon  # Small value to avoid numerical issues
       
        
    
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        # print(f"🔍 Original scores - min: {scores.min().item():.3f}, max: {scores.max().item():.3f}")
        
        # Create mask for finite values
        print(f"\n\n\n ############ the tendency is {self.tendency} \n\n\n ")
        ...
        if finite_mask.any():
            ...
            # print(f"🔍 Processing {finite_mask.sum()} finite values out of {scores.numel()} total")
            
            # Store original scale information
            original_min = torch.min(finite_scores)
            original_max = torch.max(finite_scores)
            
            # Apply your normalization only to finite values
            min_finite = torch.min(finite_scores)
            finite_shifted = finite_scores - min_finite
            max_finite = torch.max(finite_shifted)
            finite_normalized = finite_shifted / (max_finite + self.epsilon)
            
            # Apply power transformation only to finite values
            finite_processed = torch.pow(finite_normalized + self.epsilon, 1.0 / self.tendency)
            
            print(f"🔍 Finite processed - min: {finite_processed.min().item():.3f}, max: {finite_processed.max().item():.3f}")
            
            # Convert back to original scale
            # Scale from [0, 1] back to [original_min, original_max]
            finite_rescaled = finite_processed * (original_max - original_min) + original_min
            
            # Create output tensor: start with original scores (preserves -inf)
            scores_processed = scores.clone()
            # Replace only the finite values with rescaled ones
            scores_processed[finite_mask] = finite_rescaled
            
            print(f"🔍 Rescaled back - min: {finite_rescaled.min().item():.3f}, max: {finite_rescaled.max().item():.3f}")
        else:
            # No finite values to process
            scores_processed = scores.clone()
        
        # print(f"🔍 Final - finite values: {torch.isfinite(scores_processed).sum()}, -inf values: {torch.isinf(scores_processed).sum()}")
...
class REaLSampler:
    def __init__(
        ...
        device="cuda",
        processor: bool = False, # Whether we should enable my processor
    ...
    def _generate(
        ...
        **generate_kwargs,
        ...
        if self.processor: #Enabling to process the logit
        
            # Ensure sampling is enabled for your logit processing to work
            # Use standard softmax (temperature = 1.0) and let alpha do the work
            generate_kwargs["do_sample"] = True
            # generate_kwargs["temperature"] = 1.0  # Standard softmax
            
            # Remove other sampling parameters to keep it simple
            # generate_kwargs.pop("top_k", None)
            # generate_kwargs.pop("top_p", None)
            
            
            # Additional parameters to ensure non-greedy sampling
            # generate_kwargs["num_beams"] = 1  
            
            # Make sure these greedy-related parameters are not set
            # generate_kwargs.pop("num_beam_groups", None)  # Remove beam search grouping
            # generate_kwargs.pop("early_stopping", None)   # Remove early stopping (beam search feature)
            ...
            # print("\n\n About to create CustomLogitsProcessor...\n\n")
            processor = CustomLogitsProcessor(tendency = self.tendency)
            # print("\n\n  CustomLogitsProcessor created! \n\n")
            logits_processor.append(processor)
            # print(f"\n\n Added to list, length now: {len(logits_processor)}\n\n")
            
            # Add logits processor to generation kwargs
...
class TabularSampler(REaLSampler):
    ...
    def __init__(
        ...
        super().__init__(
            ...
            random_state,
            device,
            processor = processor,# Whether we want to enable processor 
            tendency = tendency    # Work with processor
        )
    ...
    def sampler_from_model(rtf_model, device: str = "cuda", 
                            processor: bool = False, tendency: Optional[float] = None):
        ...
        return TabularSampler(
            ...
            device=device,
            processor = processor, # Whether enable my loggit processor or not
            tendency = tendency    # Work with the processor
        )
