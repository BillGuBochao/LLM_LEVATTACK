...
def train_and_generate_synthetic_data(base_dir: str, multipliers: list = [1]):
    """
    ...
    #             synthetic_df.dataframe().to_csv(os.path.join(ctgan_synth_dir, f"{m}x_{i}.csv"), index=False)
    ...
    #         print(f"Error training CTGAN for {dataset_name}: {str(e)}")
        ...
        try:
            ...
            for m in multipliers:
                ...
                synthetic_df.dataframe().to_csv(os.path.join(tvae_synth_dir, f"{m}x_{i}.csv"), index=False)
        ...
        #         synthetic_df.dataframe().to_csv(os.path.join(great_synth_dir, f"{m}x_{i}.csv"), index=False)
        #         print(f"Generated {m}x synthetic samples ({sample_size} rows) for {dataset_name}")
