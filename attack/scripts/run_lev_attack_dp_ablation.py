...
summary_path = os.path.join(ROOT_DIR, "dp_ablation_mia_evaluation_summary.csv")
fieldnames = [
...
for experiment_folder in sorted(os.listdir(ROOT_DIR)):
    ...
    if not all(os.path.exists(p) for p in [member_path, non_member_path, synth_base_path]):
        print(f"  Skipping {experiment_folder}: Missing required files or synth directory.")
        continue
    ...
    # Process models within the synth directory
    for model_type in sorted(os.listdir(synth_base_path)):
        ...
        for synth_file in sorted(os.listdir(model_path)):
            ...
            try:
                ...
                # CASE AGNOSTIC & ORDERING: 
                # Normalize synth headers and reorder them to match common_cols
                synth_df.columns = [c.lower() for c in synth_df.columns]
                
                # Check if all required columns exist in synth data
                missing_cols = set(common_cols) - set(synth_df.columns)
                if missing_cols:
                    print(f"      Warning: {synth_file} is missing columns: {missing_cols}")
                    # Reindex handles missing columns by filling with NaN, ensuring order is preserved
                ...
                scores_filename = f"lev_scores_{model_type}_{synth_file}"
                scores_df.to_csv(os.path.join(experiment_path, scores_filename), index=False)
                ...
                result_row = {col: (None if col not in ["experiment", "model_type", "synth_file"] else locals().get(col)) for col in fieldnames}
