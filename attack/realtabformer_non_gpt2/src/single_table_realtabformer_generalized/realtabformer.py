...
#import single_table_realtabformer_generalized
...
class REaLTabFormer:
    def __init__(
        ...
        **training_args_kwargs,
        ...
        if self.model_type == ModelType.tabular:
            ...
            # Ignore relational and finish parent first
            self._init_tabular(tabular_config) #Thomas: default None and plug in model name later?
        ...
        #self.realtabformer_version = single_table_realtabformer_generalized.__version__
    ...
    def _init_tabular(self, tabular_config):
        if tabular_config is not None:
            ...
            if 'nvidia' in self.llm_name:
                tabular_config = AutoConfig.from_pretrained(self.llm_name, num_hidden_layers = 6, trust_remote_code = True) #Thomas: possible to change configurations?
            else:
    ...
    def _init_relational(self, relational_config):
        if relational_config is not None:
            ...
            #)
            # Thomas: seq-2-seq encoders and decoders are changed to respective transformers
            relational_config = EncoderDecoderConfig(
                encoder=AutoConfig.from_pretrained(self.llm_name, num_hidden_layers = 3).to_dict(),
                decoder=AutoConfig.from_pretrained(self.llm_name, num_hidden_layers = 3).to_dict(),
                # Thomas: 
                # encoder=GPT2Config(n_layer=6).to_dict(),
                # decoder=GPT2Config(n_layer=6).to_dict(),
            )
        ...
        if self.parent_gpt2_config is not None:
            ...
            # Thomas: encoder changed to respective configuration
            relational_config.encoder = AutoConfig.from_pretrained(self.llm_name, **self.parent_gpt2_config)
    ...
    def _set_up_relational_coder_configs(self) -> None:
        ...
        # Thomas: Change to autoconfig directly?
        def _get_coder(coder_name) -> AutoConfig:
        ...
        for coder_name in ["encoder", "decoder"]:
            ...
            if (
                ...
                # Thomas: replace n_positions with max_position_embeddings for Llama config name
                and coder.max_position_embeddings < self.relational_max_length
            ):
                # Thomas: replace n_positions with max_position_embeddings
                # coder.n_positions = 128 + self.relational_max_length
                coder.max_position_embeddings = 128 + self.relational_max_length
            elif coder_name == "encoder" and getattr(
                # coder, "n_positions", getattr(coder, "max_position_embeddings")
                # Thomas: replace n_positions with max_position_embeddings
                coder, "max_position_embeddings", getattr(coder, "max_position_embeddings")
    ...
    def _fit_relational(
        ...
        in_out_idx.to_csv("in_out_idx.csv")
        in_df.to_csv("in_df.csv")
        out_df.to_csv("out_df.csv")
        #self.vocab.to_csv('vocab.csv')
        ...
        in_df.to_csv("in_df_transformed.csv")
        out_df.to_csv("out_df_transformed.csv")
        ...
        dataset = make_relational_dataset(
            ...
            # Thomas: Llama has more tokens so need to remove the max length?
        ...
        #print(dataset.map(lambda example: dict(length=len(example["labels"]))))
        self.relational_max_length = (max(dataset.map(lambda example: dict(length = len(example["labels"])))["length"]) + 1)
        ...
        #Thomas: replace EncoderDecoder with 
    ...
    def _fit_tabular(
        ...
        # Thomas: guess n_positions in GPT2config should be equivalent to max_position_embeddings
        if self.tabular_config.max_position_embeddings < len(self.vocab["column_token_ids"]):
            self.tabular_config.max_position_embeddings = 128 + len(self.vocab["column_token_ids"])

        #print(self.tabular_config)
        # self.model = GPT2LMHeadModel(self.tabular_config)
        
        # Thomas: ignore_mismatched_sizes allows the layer to adjust to our own vocab size
        # tokenizer = AutoTokenizer.from_pretrained(self.llm_name)
        self.model = AutoModelForCausalLM.from_pretrained(self.llm_name, config = self.tabular_config, ignore_mismatched_sizes = True)
    ...
    def load_from_dir(cls, path: Union[str, Path]):
        ...
        for k, v in rtf_attrs.items():
            if k == "gpt_config":
                ...
                # Thomas: change directly to AutoConfig?
                v = AutoConfig.from_dict(v)
                k = "tabular_config"
        ...
        if realtf.model_type == ModelType.tabular:
            ...
            realtf.model = AutoModelForCausalLM.from_pretrained(rtf_attrs['llm_name'], realtf.tabular_config)
        elif realtf.model_type == ModelType.relational:
